...
Les deux Shortcuts envoient ton `X-API-Key` en clair sur le tailnet. Ce
n'est pas un problème dans cette config (Tailscale chiffre déjà tout le
trafic et n'est accessible que par tes appareils), mais ne partage pas
les Shortcuts exportés avec quelqu'un d'autre — la clé y est inscrite.
