...
from bot.pipeline.dates import normalize_fr_time_words, parse_due, parse_when_to_date


def test_normalize_midi_and_minuit() -> None:
    assert normalize_fr_time_words("demain midi") == "demain 12:00"
    # "ce soir" est substitué à "aujourd'hui" (dateparser gère bien cette expression).
    assert normalize_fr_time_words("ce soir minuit") == "aujourd'hui 00:00"
    assert normalize_fr_time_words("Midi pile") == "12:00 pile"


def test_parse_due_handles_midi() -> None:
    result = parse_due("demain midi", "Europe/Paris")
...
def test_parse_due_handles_minuit() -> None:
    result = parse_due("minuit", "Europe/Paris")
    assert result is not None
...
def test_parse_due_none_when_empty() -> None:
    assert parse_due(None, "Europe/Paris") is None
    assert parse_due("", "Europe/Paris") is None


def test_normalize_hour_markers() -> None:
    # "Xh" et "XhYY" → "HH:MM" pour empêcher dateparser de lire une durée.
    assert normalize_fr_time_words("demain à 7h") == "demain à 07:00"
    assert normalize_fr_time_words("demain 7h30") == "demain 07:30"
    assert normalize_fr_time_words("vendredi 19h00") == "vendredi 19:00"
    # Pas de transformation derrière un connecteur de durée.
    assert normalize_fr_time_words("dans 7h") == "dans 7h"
    assert normalize_fr_time_words("il y a 2h") == "il y a 2h"
...
def test_parse_due_handles_explicit_hour() -> None:
    ...
    result = parse_due("demain à 7h", "Europe/Paris")
    assert result is not None
...
def test_parse_due_handles_hour_with_minutes() -> None:
    result = parse_due("demain 7h30", "Europe/Paris")
    assert result is not None
    assert result.hour == 7
...
def test_parse_when_to_date_prefers_past() -> None:
    ...
    for day in (1, 5, 15, 28):
        resolved = parse_when_to_date(f"le {day}", "Europe/Paris")
...
def test_parse_when_to_date_keeps_requested_day_in_previous_month() -> None:
    ...
    recul est fait manuellement par `parse_when_to_date`. Le jour demandé
...
def test_parse_when_to_date_defaults_to_today_when_empty() -> None:
    tz = ZoneInfo("Europe/Paris")
    assert parse_when_to_date(None, "Europe/Paris") == datetime.now(tz).date()
